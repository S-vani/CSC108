...
def is_palindrome_string(string: str) -> bool:
    ...
    Restrictions: you must use your "is_palindrome" function from lab 3 as a
                  ...
                  for this function.
    >>> is_palindrome_string("test string test")
    True
    >>> is_palindrome_string("test string strong")
    False
    >>> is_palindrome_string("hello world hey")
    True
    """
    string += " "
    final_string = ""
    ...
    for letter in range(len(string)):
        if not string[letter].isalpha():
            final_string += string[letter - len_of_current]
            len_of_current = -1
        len_of_current += 1
    return is_palindrome(final_string)
...
def reverse_sentence(s: str) -> str:
    ...
    while i < len(s):
        if s[i].isupper():
            ...
            segment = ""
            while i < len(s) and (i == start_index or not s[i].isupper()):
                segment += s[i]
                i += 1
